    ...
    methods: [
      {
        ...
        items: [
          ...
          },
          {
            name: "Object.assign()",
            description:
              "Копирует значения всех перечисляемых свойств из одного или нескольких объектов в целевой объект.",
          ...
          },
          {
            name: "Object.seal()",
            description:
              "Запечатывает объект: нельзя добавлять/удалять свойства, но можно изменять существующие.",
          ...
          },
          {
            ...
            example:
              'const proto = {greet() {return "hi"}}; const obj = Object.create(proto); obj.greet(); // "hi"',
          ...
          },
          {
            name: "hasOwnProperty()",
            description:
              "Проверяет, содержит ли объект собственное свойство с указанным именем (на прототипе).",
  ...
  const highlightText = (text: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === searchQuery.toLowerCase() ? (
        <span key={i} className="bg-yellow-200 text-yellow-900 px-1 rounded">
          {part}
        </span>
      ) : (
        part
      ...
      <div className="container mx-auto px-4 py-8 flex-grow">
        <div className="mb-4">
          ...
          <p className="text-muted-foreground text-sm sm:text-base">
            Справочник по методам работы с различными типами данных
          </p>
        </div>
                ...
                {DATA_TYPES.map((type) => (
                  ...
                  >
                    <div className={`p-1 rounded-lg ${type.color}`}>
                      <type.icon
                        className={`h-4 w-4 ${selectedType === type.id ? "text-white" : "text-gray-500"}`}
                      />
                    </div>
                    <span>{type.name}</span>
                  </button>
                    ...
                    <div key={type.id}>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="ml-2 text-xs text-gray-500 font-sans">
                          Найдено методов:{" "}
                          {type.methods.reduce((sum, category) => sum + category.items.length, 0)}
                        </span>
                      </div>
                      <Accordion
                        type="multiple"
                            ...
                            <AccordionTrigger className="px-3 py-2 bg-purple-50 hover:bg-purple-100 text-sm font-medium font-sans flex items-center justify-between">
                              <div className="flex flex-col items-start">
                                ...
                                {category.items && category.items.length > 0 && (
                                  <span className="mt-0.5 text-xs text-gray-400 truncate max-w-full">
