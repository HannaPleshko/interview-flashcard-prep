  ...
  // html
  {
    ...
    questions: [
      ...
      },
      {
        ...
        answer: "sessionstorage — хранит информацию в браузере, пока не закрыта вкладка, после ее закрытия — информация удаляется. localstorage — хранит информацию в браузере даже после того, как вкладка закрыта, не имеет срока жизни. cookies — хранят информацию, имеют срок жизни (expires), отправляются на сервер с запросом.",
        difficulty: "easy"
      },
      {
        id: "html-24",
      ...
      {
        id: "html-31",
        question: "Можно ли использовать более одного атрибута \"class\" для одного элемента, и если да, то как это делается?",
        answer: "НЕЛЬЗЯ использовать несколько атрибутов class для одного элемента. Но можно использовать более одного атрибуте \"class\" для одного элемента. Для этого нужно добавить еще один класс через пробел. Например: <div class=\"class1 class2\">.",
        difficulty: "easy"
      },
      {
        id: "html-32",
        question: "Можно ли использовать более одного тега MAIN на странице и что будет происходить с их содержимым?",
        answer: "Нет, использовать более одного тега MAIN на странице нельзя согласно спецификации HTML5, и хотя все содержимое внутри дополнительных тегов MAIN будет визуально отображаться в браузере, это приведет к серьезным проблемам: нарушению семантической структуры документа, ошибкам валидации HTML, ухудшению доступности для скринридеров и потенциальным проблемам с SEO, поскольку вспомогательные технологии не смогут правильно определить основное содержание страницы.",
        difficulty: "easy"
      },
      {
        id: "html-33",
        question: "Что такое семантические теги и зачем они нужны?",
        answer: "Семантические теги - это теги, которые используются для определения содержимого веб-страницы по его значениям, а не для форматирования внешнего вида. Они помогают поисковым системам и другим программам лучше понимать содержимое страницы, что повышает ее доступность и SEO-оптимизацию. Примеры: <header>, <nav>, <main>, <section>, <article>, <aside>, <footer>.",
        difficulty: "easy"
      },
    ]
  },
    ...
    questions: [
      {
        ...
        answer: "<strong>CSS</strong> — это язык, который определяет, как элементы веб-страницы будут выглядеть, отвечая за их стиль, форматирование, цвета и расположение. Он используется для оформления документов, написанных на языках разметки, таких как HTML и XML, позволяя разделить структуру документа от его представления и сделать сайт визуально привлекательным и удобным для пользователя.",
        difficulty: "easy"
      },
      {
        id: "css-2",
...
`,
      ...
      {
        id: "css-13",
        question: "Как работает система сеток в CSS Grid и какие есть основные свойства Grid контейнера?",
        answer: `
...
• Может схлопываться с margin соседних элементов<br><br>    `,
      ...
      },
      {
        ...
        answer: "Свойство box-sizing в CSS определяет, как рассчитываются размеры элемента, включая его паддинг (padding) и границу (border). Оно помогает контролировать, как будет вычисляться общая ширина и высота элемента при добавлении внутренних отступов и границ.",
        difficulty: "easy"
      },
      {
        id: "css-18",
...
• Не зависит от родительских элементов, более предсказуем<br><br>`,
      ...
      {
        id: "css-23",
        question: "Как работает наследование размеров при использовании em и почему это может быть проблемой?",
        answer: `
  ...
  // JS
  {
    ...
    questions: [
      ...
      
      {
        ...
        answer: "<strong>WeakMap</strong> — коллекция пар ключ-значение, где ключи только объекты и слабо удерживаются:<br/><br/>const wm = new WeakMap();<br/>let obj = { name: 'Иван' };<br/>wm.set(obj, 'some data');<br/>obj = null; // объект удалится из WeakMap автоматически<br/><br/><strong>WeakSet</strong> — коллекция уникальных объектов со слабыми ссылками:<br/><br/>const ws = new WeakSet();<br/>let obj = { name: 'Иван' };<br/>ws.add(obj);<br/>obj = null; // объект удалится из WeakSet<br/><br/><strong>Особенности:</strong><br/>• Ключи/значения только объекты<br/>• Не препятствуют сборке мусора<br/>• Нельзя итерировать<br/>• Нет свойства size<br/>• Нет метода clear()<br/><br/><strong>Применение:</strong><br/>• Хранение приватных данных<br/>• Кеширование без утечек памяти<br/>• Метаданные для объектов<br/><br/>// Приватные данные<br/>const privateData = new WeakMap();<br/>class User {<br/>&nbsp;&nbsp;constructor(name) {<br/>&nbsp;&nbsp;&nbsp;&nbsp;privateData.set(this, { name });<br/>&nbsp;&nbsp;}<br/>}",
        difficulty: "hard"
      },
