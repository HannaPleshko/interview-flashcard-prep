  ...
  // react
  {
    ...
    questions: [
      ...
      },
      {
        id: "rct-2",
        question:
          "Объясните концепцию Виртуального DOM.",
        answer:`<strong>Виртуальный DOM</strong> — это облегчённое, хранящееся в памяти представление реальной DOM. React использует его для оптимизации работы интерфейса.<br/><br/>

Когда меняется состояние или данные компонента, React сначала обновляет <strong>виртуальную DOM</strong>, а не реальные элементы страницы.<br/>
Затем с помощью <strong>diffing-алгоритма</strong> вычисляется разница между старой и новой виртуальной DOM.<br/>
После этого React обновляет <strong>только те части реальной DOM</strong>, которые действительно изменились, что делает рендеринг быстрым и эффективным.<br/><br/>

Такой подход позволяет создавать динамичные интерфейсы с высокой производительностью, минимизируя количество дорогих операций с реальной DOM.
`,
        difficulty: "medium",
      },
      {
        ...
        answer: `Основное различие между <strong>классовыми</strong> и <strong>функциональными</strong> компонентами заключается в способе их определения и работе со состоянием и жизненным циклом.<br/><br/>

<strong>Классовые компоненты:</strong><br/>
— Определяются как ES6-классы и расширяют <code>React.Component</code>.<br/>
— Используют метод <code>render()</code> для возврата JSX.<br/>
— Поддерживают состояние через <code>this.state</code> и <code>this.setState()</code>.<br/>
— Имеют доступ к методам жизненного цикла (componentDidMount, componentDidUpdate и др.).<br/><br/>
...
<strong>Функциональные компоненты:</strong><br/>
— Определяются как обычные JS-функции.<br/>
— Принимают пропсы как аргументы и возвращают JSX напрямую.<br/>
— Ранее были «безд状態», но с появлением <strong>хуков</strong> (useState, useEffect и др.) получили возможность управлять состоянием и использовать функционал жизненного цикла.<br/><br/>
...
Функциональные компоненты проще, легче читаются и тестируются, и являются рекомендуемым способом написания компонентов в современных приложениях, если нет необходимости использовать классовый синтаксис.
`,
        difficulty: "medium",
      },
      {
        ...
        answer: `<strong>JSX</strong> — это расширение синтаксиса JavaScript, позволяющее использовать HTML-подобную разметку внутри кода JavaScript.<br/><br/>
      
      Без JSX создание элементов в React выглядит так:<br/>
      <pre>
      ...
      С JSX тот же элемент можно записать проще и нагляднее:<br/>
      <pre>
      const someElement = (
      ...
      JSX — это своего рода синтаксический сахар, который упрощает разработку и делает код более читаемым.`,
        difficulty: "easy",
      },
      
      {
        id: "rct-5",
        question: "Какие хуки вы знаете?",
        answer:`<strong>React.useState</strong> — управления состоянием в функциональных компонентах<br/>
<strong>useEffect</strong> — выполнения побочных эффектов (загрузка данных, подписки и т.д.)<br/>
<strong>useContext</strong> — доступа к значению контекста React<br/>
<strong>useRef</strong> — создания изменяемых ссылок, сохраняющихся между рендерами<br/>
<strong>useCallback</strong> — мемоизации функций для оптимизации ререндеров<br/>
<strong>useMemo</strong> — кэширования вычислений для повышения производительности<br/>
<strong>useReducer</strong> — управления состоянием через reducer (аналогично Redux-подходу)<br/>
<strong>useLayoutEffect</strong> — синхронных побочных эффектов после мутаций DOM
`,
        difficulty: "easy",
      },
      {
        id: "rct-6",
        question: "Как выполнить рендеринг элементов массива?",
        answer:`Для рендеринга элементов массива в React обычно используют метод <strong>map()</strong>, который возвращает новый массив JSX-элементов.<br/><br/>

<strong>Пример статического списка:</strong><br/>
Используем <code>map()</code>, чтобы преобразовать строки в элементы <code>&lt;li&gt;</code>:
<br/>
...
<strong>Динамические списки и ключи:</strong><br/>
При выводе динамических данных необходимо указывать <strong>key</strong> — уникальный идентификатор элемента.  
Это помогает React эффективно отслеживать изменения списка (добавление, удаление, перестановки).
...
Таким образом, рендеринг массива выполняется через <strong>map()</strong>, а для динамических данных обязательно использование <strong>key</strong> для корректной и оптимальной работы React.
`,
        difficulty: "medium",
      },
      {
        id: "rct-7",
        question: "Что такое методы жизненного цикла компонента?",
        answer:`<strong>Методы жизненного цикла</strong> — это специальные методы классовых компонентов React, которые позволяют выполнять код на разных этапах существования компонента: при создании, обновлении и удалении.<br/><br/>

Основные методы жизненного цикла:<br/><br/>

<strong>constructor</strong> — вызывается первым при создании компонента. Используется для инициализации состояния и привязки обработчиков. В функциональных компонентах аналог — <strong>useState</strong>.<br/><br/>

<strong>render</strong> — метод, возвращающий JSX и отвечающий за отображение компонента.<br/><br/>

<strong>componentDidMount</strong> — запускается сразу после первого рендера. Применяется для загрузки данных, подписок, работы с DOM.<br/><br/>

<strong>componentDidUpdate</strong> — вызывается при обновлении пропсов или состояния. Подходит для выполнения побочных эффектов после изменения данных.<br/><br/>

<strong>componentWillUnmount</strong> — вызывается перед удалением компонента из DOM. Используется для очистки: отписки от событий, остановки таймеров и т.д.<br/><br/>

Некоторые старые методы (например, <em>componentWillMount</em>, <em>componentWillReceiveProps</em>, <em>componentWillUpdate</em>) считаются устаревшими, а в функциональных компонентах их заменяют хуки, такие как <strong>useEffect</strong>.
`,
        difficulty: "easy",
      },
      {
        id: "rct-8",
        question: "В чем заключаются особенности использования useState?",
        answer: `<strong>useState</strong> возвращает текущее значение состояния и функцию для его обновления.<br/><br/>
      
      <pre>
      const [value, setValue] = useState('Some state');
      </pre>
      
      При первом рендеринге <strong>value</strong> будет равно значению, переданному в useState.<br/>
      Для изменения состояния используется функция <strong>setValue</strong>, которая:<br/>
      — принимает новое значение состояния и ставит компонент в очередь на повторный рендеринг;<br/>
      — может принимать функцию обратного вызова, получающую предыдущее значение состояния, что удобно для вычислений на основе текущего состояния.`,
        difficulty: "medium",
      },
      {
        id: "rct-9",
        question: "В чем заключаются особенности использования useEffect?",
        answer: `<strong>useEffect</strong> позволяет выполнять побочные эффекты в функциональных компонентах, такие как мутации, подписки, таймеры или логирование.<br/><br/>
      
      Побочные эффекты не должны выполняться во время рендеринга, чтобы избежать ошибок и несоответствий в интерфейсе. Вместо этого useEffect запускается после фазы фиксации DOM.<br/><br/>
      
      Если передать массив зависимостей вторым аргументом, эффект будет выполняться только при изменении этих зависимостей.<br/><br/>
      ...
      В этом примере эффект выполнится один раз после первого рендера компонента.`,
        difficulty: "medium",
      },
      {
        id: "rct-10",
        question: "Как отследить размонтирование функционального компонента?",
        answer: `<strong>Размонтирование</strong> функционального компонента можно отслеживать с помощью функции очистки, возвращаемой из <strong>useEffect</strong>.<br/><br/>
      
      Функция очистки выполняется перед удалением компонента из DOM и позволяет освобождать ресурсы, отменять подписки или таймеры, предотвращая утечки памяти. Также она вызывается перед повторным выполнением эффекта при обновлении зависимостей.<br/><br/>
      ...
      Таким образом, возвращаемая функция используется для корректного завершения работы и очистки ресурсов компонента.`,
        difficulty: "medium",
      },
      {
        id: "rct-11",
        question: "Что такое менеджер состояний и с какими из них вы работали?",
        answer: `<strong>Менеджер состояний</strong> — это инструмент или библиотека для управления состоянием приложения. Он предоставляет централизованное хранилище для хранения данных, доступных и изменяемых из разных компонентов.<br/><br/>
      
      Менеджер состояния помогает:<br/>
      — отделять данные и связанную с ними логику от компонентов;<br/>
      — избегать запутанности кода при передаче состояния между вложенными компонентами;<br/>
      — иметь единое место для управления глобальным состоянием.<br/><br/>
      
      В React роль менеджера состояния может выполнять <strong>React Context</strong>, но для крупных приложений часто используют библиотеки, такие как <strong>Redux</strong> или <strong>MobX</strong>.`,
        difficulty: "medium",
      },
      {
        id: "rct-12",
        question: "Что такое reducer в Redux и какие параметры он принимает?",
        answer: `<strong>Reducer</strong> — это чистая функция, которая управляет состоянием в Redux. Она принимает два параметра:<br/><br/>
      
      1. <strong>state</strong> — текущее состояние приложения (или часть состояния, за которую отвечает данный редуктор).<br/>
      2. <strong>action</strong> — объект действия, который содержит как минимум поле <code>type</code>, описывающее, какое изменение нужно выполнить.<br/><br/>
      
      Внутри редуктора обычно используют конструкцию <code>switch</code> по <code>action.type</code> для определения изменений состояния. Редуктор возвращает новое состояние, не мутируя старое.<br/><br/>
      ...
      },
      {
        ...
        answer: `<strong>Действие (action)</strong> — это простой объект JavaScript, который описывает, какое изменение нужно выполнить в состоянии. Обязательно должно содержать поле <code>type</code>:<br/><br/>
      ...
      Опционально можно добавить поле <strong>payload</strong> с данными, которые нужны для обновления состояния:<br/><br/>
      ...
      Чтобы изменить состояние, необходимо вызвать функцию <strong>dispatch</strong> и передать ей это действие. Reducer получит действие и вернет новое состояние.`,
        difficulty: "medium",
      },
      {
        id: "rct-14",
        question: "Какой паттерн реализует Redux?",
        answer: `<strong>Redux</strong> реализует паттерн <strong>Flux</strong>.<br/><br/>
      
      Flux — это предсказуемый шаблон управления состоянием приложения, который использует однонаправленный поток данных и централизованное хранилище для состояния. Такой подход помогает контролировать изменения состояния и делает поведение приложения более предсказуемым.`,
        difficulty: "medium",
      },
      {
        id: "rct-15",
        question: "В чем разница между Redux и Mobx?",
        answer: `<strong>Redux</strong> — это библиотека управления состоянием с однонаправленным потоком данных и принципом неизменяемости. Требует явного обновления состояния и часто сопровождается шаблонным кодом, но хорошо интегрируется с React.<br/><br/>
      
      <strong>MobX</strong> предоставляет более гибкий и простой API: состояние можно изменять напрямую, а библиотека автоматически отслеживает изменения и обновляет интерфейс, повышая производительность.<br/><br/>
      
      Выбор между Redux и MobX зависит от размера проекта, требований к предсказуемости состояния и предпочтений команды разработчиков.`,
        difficulty: "medium",
      },
      {
        id: "rct-16",
        question: "Что такое пробрасывание пропсов (props drilling)?",
        answer: `<strong>Пробрасывание пропсов (props drilling)</strong> — это процесс передачи реквизитов через несколько уровней вложенных компонентов, даже если промежуточные компоненты не используют эти пропсы напрямую. Это может сделать код запутанным и сложным для поддержки.<br/><br/>
      ...
      const ChildA = ({ data }) =&gt; &lt;ChildB data={data} /&gt;;
      
      // Целевой компонент
      const ChildB = ({ data }) =&gt; &lt;div&gt;{data}&lt;/div&gt;;
      </pre>
      
      В этом примере <code>data</code> передается через ChildA, который сам её не использует. При глубокой вложенности такой подход усложняет код.<br/><br/>
      
      Для решения проблемы используют <strong>React Context</strong> или библиотеки управления состоянием (Redux, MobX), чтобы вложенные компоненты могли получать данные напрямую, без необходимости пробрасывать пропсы через каждый уровень.<br/><br/>
      
      <pre>
      // Создание контекста
      const DataContext = React.createContext();
      
      // Родительский компонент с провайдером
      const Parent = () =&gt; {
        const data = "Hello World!";
        return &lt;DataContext.Provider value={data}&gt;&lt;ChildA /&gt;&lt;/DataContext.Provider&gt;;
      };
      
      // Получение данных в глубоком компоненте
      const ChildB = () =&gt; {
      ...
      {
        id: "rct-17",
        question: "Как работает условный рендеринг элементов?",
        answer: `<strong>Условный рендеринг</strong> в React позволяет отображать элементы в зависимости от состояния или условий. Для этого можно использовать логические и тернарные операторы, а также обычные условные конструкции.<br/><br/>
      
      <em>Примеры:</em><br/>
      
      Логический оператор AND:<br/>
      <pre>
      return (
        &lt;div&gt;
      ...
      Таким образом, React позволяет гибко управлять отображением элементов на основе условий.`,
        difficulty: "medium",
      },
      {
        id: "rct-18",
        question: "Для чего и как используется useMemo?",
        answer: `<strong>useMemo</strong> — это хук React, который используется для кэширования результатов вычислений и оптимизации производительности.<br/><br/>
      
      В useMemo передается функция, которая выполняет вычисления, и массив зависимостей. Значение будет пересчитано только в случае изменения хотя бы одной зависимости. Это позволяет избегать дорогостоящих операций при каждом рендере.<br/><br/>
      
      <pre>
      const memoValue = useMemo(() =&gt; computeFunc(paramA, paramB), [paramA, paramB]);
      </pre>
      
      В этом примере <code>computeFunc</code> будет вызвана только если <code>paramA</code> или <code>paramB</code> изменятся.`,
        difficulty: "medium",
      },
      {
        id: "rct-19",
        question: "Для чего используется useCallback и как он работает?",
        answer: `<strong>useCallback</strong> — это хук React, который возвращает мемоизированную версию коллбэк-функции. Она изменяется только в случае изменения значений зависимостей, переданных во втором аргументе.<br/><br/>
      
      Хук полезен при передаче функций в оптимизированные дочерние компоненты, которые используют проверку ссылочного равенства (===), чтобы предотвращать лишние рендеры.<br/><br/>
      
      Преимущества useCallback:<br/>
      — ссылка на функцию остаётся стабильной между рендерами, если зависимости не изменились;<br/>
      — уменьшает количество лишних отрисовок, повышая производительность.<br/><br/>
      ...
      },
      {
        ...
        answer: `<strong>useMemo</strong> используется для кэширования <strong>результата вычислений</strong> и возвращает вычисленное значение. Если зависимости не изменились, значение берется из кэша и не пересчитывается при каждом рендере.<br/><br/>
      
      <strong>useCallback</strong> используется для кэширования <strong>самой функции</strong>. Он возвращает ту же функцию между рендерами, если зависимости не изменились, что помогает предотвращать лишние ререндеры дочерних компонентов.<br/><br/>
      
      Иными словами:<br/>
      — useMemo → кэширует результат вычислений;<br/>
      — useCallback → кэширует функцию.`,
        difficulty: "medium",
      },
      {
        id: "rct-21",
        question: "Как используют React Context?",
        answer: `<strong>React Context</strong> позволяет передавать данные через дерево компонентов без необходимости пробрасывать пропсы на каждом уровне (prop drilling). Он полезен для глобального состояния, к которому могут обращаться любые компоненты внутри провайдера.<br/><br/>
      
      Основные части Context API:<br/>
      1. <strong>createContext</strong> — создаёт новый объект контекста.<br/>
      2. <strong>Context.Provider</strong> — оборачивает компоненты и предоставляет значение контекста.<br/>
      3. <strong>Context.Consumer</strong> или <strong>useContext</strong> — используются для получения значения контекста в любом компоненте, находящемся внутри провайдера.<br/><br/>
      
      Использование контекста помогает упростить структуру кода, избегая глубокого пробрасывания пропсов, и облегчает управление состоянием на более высоком уровне.`,
        difficulty: "medium",
      },
      {
        id: "rct-22",
        question: "Для чего используется useContext и как он работает?",
        answer: `<strong>useContext</strong> — это хук React, который позволяет получать значение из контекста без необходимости пробрасывать пропсы через каждый уровень компонентов.<br/><br/>
      
      Когда компонент использует <strong>useContext</strong>, он автоматически подписывается на изменения значения контекста. При обновлении значения контекста компонент будет перерисован. Для оптимизации дорогих ререндеров можно использовать мемоизацию.<br/><br/>
      ...
      Таким образом, useContext упрощает доступ к глобальным данным, таким как тема или язык интерфейса, без пробрасывания пропсов через все уровни компонентов.`,
        difficulty: "medium",
      },
      {
        id: "rct-23",
        question: "Для чего используется useRef и как он работает?",
        answer: `<strong>useRef</strong> — это хук React, который возвращает объект-ссылку (ref), сохраняющий значение между рендерами компонента.<br/><br/>
      
      Особенности useRef:<br/>
      — начальное значение устанавливается через аргумент хука;<br/>
      — объект сохраняется между рендерами, изменение значения не вызывает перерисовку;<br/>
      — часто используется для прямого доступа к DOM-элементам или хранения изменяемых значений без вызова рендера.<br/><br/>
      ...
      Таким образом, useRef позволяет работать с элементами DOM в императивном стиле и хранить значения между рендерами без повторного обновления компонента.`,
        difficulty: "medium",
      },
      {
        id: "rct-24",
        question: "Что такое React.memo()?",
        answer: `<strong>React.memo()</strong> — это компонент высшего порядка, используемый для оптимизации производительности. Он запоминает результат рендера функционального компонента и повторно использует его, если входные реквизиты (props) не изменились.<br/><br/>
      
      Особенности React.memo():<br/>
      — предотвращает лишние рендеры при неизменных пропсах;<br/>
      — не предотвращает ререндер при изменении состояния через <code>useState</code>, <code>useReducer</code> или контекста через <code>useContext</code>;<br/>
      — полезен для компонентов, которые часто рендерятся с одинаковыми данными.<br/><br/>
      ...
      Таким образом, React.memo помогает повысить производительность, сокращая количество ненужных рендеров.`,
        difficulty: "medium",
      },
      {
        id: "rct-25",
        question: "Зачем нужны ключи в списках при использовании map()?",
        answer: `<strong>Ключи</strong> помогают React отслеживать изменения в списках. Они позволяют эффективно обновлять элементы при добавлении, удалении или изменении элементов массива.<br/><br/>
      
      Без ключей React не сможет правильно сопоставить элементы с предыдущим рендером, что может снизить производительность и вызвать ошибки при обновлении DOM.<br/><br/>
      
      При использовании <code>map()</code> обязательно указывайте уникальные ключи для каждого элемента, предпочтительно на основе уникальных идентификаторов:<br/><br/>
      ...
      Таким образом, ключи помогают React точно определять, какой элемент изменился, добавился или удалился.`,
        difficulty: "medium",
      },
      {
        id: "rct-26",
        question: "Как получить доступ к элементу DOM?",
        answer: `<strong>Доступ к DOM-элементу</strong> в React осуществляется через ссылку (<code>ref</code>). Можно использовать <code>React.createRef()</code> или хук <code>useRef()</code>.<br/><br/>
      
      Шаги:<br/>
      1. Создаём ref с помощью useRef или createRef.<br/>
      2. Присоединяем ref к элементу через атрибут <code>ref</code>.<br/>
      3. Обращаемся к DOM-элементу через <code>ref.current</code>.<br/><br/>
      ...
      Таким образом, useRef позволяет работать с DOM в императивном стиле внутри функциональных компонентов.`,
        difficulty: "medium",
      },
      {
        id: "rct-27",
        question: "Какие архитектурные решения для React вы знаете?",
        answer: `<strong>Популярные архитектурные решения и паттерны для React:</strong><br/><br/>
      
      1. <strong>MVC (Model-View-Controller)</strong> — традиционный паттерн, разделяющий приложение на модель, представление и контроллер. React чаще используется для слоя View, визуализируя интерфейс, а Model и Controller реализуются с помощью других библиотек.<br/><br/>
      
      2. <strong>Flux</strong> — архитектура от Facebook для React-приложений, основанная на однонаправленном потоке данных. Облегчает понимание кода и отладку компонентов.<br/><br/>
      
      3. <strong>Atomic Design</strong> — методология проектирования интерфейсов, разделяющая UI на мелкие переиспользуемые компоненты (атомы, молекулы, организмы), которые можно комбинировать для создания сложных интерфейсов.<br/><br/>
      
      4. <strong>Паттерн «Контейнер и компонент»</strong> — разделяет логику и представление. Компоненты отвечают за визуализацию, контейнеры — за бизнес-логику и управление состоянием.<br/><br/>
